...
```{r}
# Using indexing
lebron_points_indexing <- player_info[[2]][[4]]
...
```{r}
# calculate points per game for each player and store in a list
points_per_game <- list()
for (player in names(player_info)) {
  points <- player_info[[player]]$points
  games_played <- player_info[[player]]$games_played
...
mat_rand_rounded <- apply(mat_rand, 1:2, round_value)
...
result <- sum_of_dig(935068)
print(result)  # Should return 31 (9 + 3 + 5 + 0 + 6 + 8)
